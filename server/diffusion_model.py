...
def savetoS3Bucket(image_path):
    ...
    s3.meta.client.upload_file(Filename=image_path, Bucket='', Key=image_path)
...
def do_run(model, model_params, model_list, model_config, clip_model, clip_size, device, diffusion, folder_name, prompt_num, session):
    ...
    Path(f'{session}/{folder_name}/{prompt_num}').mkdir(parents=True, exist_ok=True)
        ...
        for j, sample in enumerate(samples):
            if j % 1 == 0 or cur_t == 0:
                for k, image in enumerate(sample['pred_xstart']):
                    ...
                    folder_path = f'{session}/{folder_name}/{prompt_num}/{filename}'
...
def define_model(clip_input, folder_name, prompt_num, session, cutn, clip_guidance_scale, tv_scale, img_size, num_steps):
    # Model settings
    ...
    torch.cuda.empty_cache()
    gc.collect()
    do_run(model, model_params, model_list, model_config, clip_model, clip_size, device, diffusion, folder_name, prompt_num, session)
